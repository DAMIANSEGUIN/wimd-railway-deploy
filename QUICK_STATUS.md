...
- ⚠️ **Automation gap** – pushes to `origin/main` do not trigger Render deploys; manual dashboard trigger required.

## Key Questions

1. What commit/hash is actually running in Render, and does `/config` read the same env source as the backend?
2. Which deployment configuration file (nixpacks, render.toml/json, Procfile) is authoritative going forward?
...
- **Option A – Pause Day 2, finish deployment fixes**: Focus entirely on schema mismatch, verification tooling, and Render automation before writing new code.
