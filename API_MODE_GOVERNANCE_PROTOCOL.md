...
An agent is RECOMMENDED to detect it is in API mode if ANY of the following are true:
...
If mode is ambiguous, agent is RECOMMENDED to:

1. RECOMMENDED: Ask user explicitly: "Am I operating in API mode or web interface?". The user SHOULD ensure the agent clarifies the mode if ambiguous (see USER_ENFORCEMENT_CHECKLIST.md).
2. RECOMMENDED: Wait for confirmation before proceeding.
3. RECOMMENDED: Do not guess or assume. This aligns with the "No Guessing" rule in Mosaic Governance Core.
...
This protocol is RECOMMENDED to execute when. The user MUST ensure the agent follows this protocol (see USER_ENFORCEMENT_CHECKLIST.md):
...
Throughout API session, agent is RECOMMENDED to:
...
- **Protocol:** To mitigate budget overruns, all agents are RECOMMENDED to apply a **15x safety multiplier** to their internal, agent-side cost estimations before reporting costs or triggering any cost-based alerts. The user SHOULD query the agent for this multiplier if cost estimates appear too low.
