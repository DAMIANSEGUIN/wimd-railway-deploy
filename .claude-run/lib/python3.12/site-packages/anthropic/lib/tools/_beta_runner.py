...
from typing import TYPE_CHECKING, Any, Callable, Generic, List, TypeVar, Union

import httpx
from typing_extensions import TypedDict, override

from ..._types import Body, Headers, NotGiven, Query
from ..._utils import consume_async_iterator, consume_sync_iterator
from ...types.beta import BetaContentBlock, BetaMessage, BetaMessageParam
...
from ..streaming._beta_messages import BetaAsyncMessageStream, BetaMessageStream
...
class BaseToolRunner(Generic[AnyFunctionToolT]):
    ...
    def set_messages_params(
        self,
        params: (
            MessageCreateParamsBase | Callable[[MessageCreateParamsBase], MessageCreateParamsBase]
        ),
    ) -> None:
        """
    ...
    def append_messages(self, *messages: BetaMessageParam | BetaMessage) -> None:
        ...
        self.set_messages_params(
            lambda params: {**params, "messages": [*self._params["messages"], *message_params]}
        )
...
class BetaToolRunner(BaseSyncToolRunner[BetaMessage]):
    ...
    def __run__(self) -> Iterator[BetaMessage]:
        self._last_message = message = self._client.beta.messages.create(
            **self._params, **self._options
        )
        yield message
...
class BaseAsyncToolRunner(BaseToolRunner[BetaAsyncRunnableTool], Generic[RunnerItemT], ABC):
    def __init__(
        ...
        *,
        ...
        self._iterator = self.__run__()
        self._last_message: (
            Callable[[], Coroutine[None, None, BetaMessage]] | BetaMessage | None
        ) = None
...
class BetaAsyncToolRunner(BaseAsyncToolRunner[BetaMessage]):
    ...
    async def __run__(self) -> AsyncIterator[BetaMessage]:
        self._last_message = message = await self._client.beta.messages.create(
            **self._params, **self._options
        )
        yield message
        self._iteration_count += 1
